function [Uout,Gout,rank] = lomac_01(Uin,Gin,tol,w1,w2,w3,Nx,Ny,Nz,dx,dy,dz,rhoM,JxM,JyM,JzM,xvals,yvals,zvals)
...
% Compute f1
Vx_f1 = w1.*[ones(Nx,1),xvals];
Vy_f1 = w2.*[ones(Ny,1),yvals];
Vz_f1 = w3.*[ones(Nz,1),zvals];
S_f1 = zeros(2,2,2);
...
S_fM(1,1,2) = JzM/(dx*dy*dz*sum(w1)*sum(w2)*sum((zvals.^2).*w3));% Compute fM-PN(trun(f2)).
